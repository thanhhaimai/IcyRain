...
import java.lang.Math; //not sure why this is showing up red to me - jre / android SDK thing?


/*
 * Sidd - 
 * Currently this gesture handler is abstracted away from how the data from the glove is actually received.
 * However the data is acquired, simply pass in pitch roll and yaw into the methods.
 */

public class GestureHandler {
	
	//set these to determine the leeway for how far apart degree measurements are to be considered "the same" to account for jitter
	...
	//some error "Implicit super constructor Object() is undefined. Must explicitly invoke another constructor"
	//something to do with jre lib or android SDK not sure, normal java this constructor should be fine.
	...
	public void CalibrateUpright(){
		...
		while(count<=calibrationCountMax){
			...
			pitchValues[count] = (double)pitch;
			pitchTotal += (double)pitch;
			
			rollValues[count] = (double)roll;
			rollTotal += (double)roll;
			
			yawValues[count] = (double)yaw;
			yawTotal += (double)yaw;
			
			count++;
		}
		
		double pitchMean = pitchTotal / (double)calibrationCountMax;
		double rollMean = rollTotal / (double)calibrationCountMax;
		double yawMean = yawTotal / (double)calibrationCountMax;
		pitchTotal = 0;
		rollTotal = 0;
		yawTotal = 0;
	...
	public boolean CheckStillHand(double p, double r, double y){
		UpdateData(p, r, y); //once UpdateData is implemented accordingly, delete this line so that Update is called once and only once for gesture checking
		
		if(Math.abs(pitch-oldPitch)<PITCH_LEEWAY && Math.abs(roll-oldRoll)<ROLL_LEEWAY && Math.abs(yaw-oldYaw)<YAW_LEEWAY){
			handIsStill = true;
			return true;
		}else{
			handIsStill = false;
	...
	/*
	 * This method relies on a wave being defined as an oscillation of the wrist rolling between two positions
	...
	public boolean CheckWave(double p, double r, double y){
		...
		//UpdateData(p, r, y); after update is implemented, this line should be called to look for changes in roll through updating rollIncreasing/Decreasing
		if(!CheckUprightHand(p, r, y)){ //update is getting called here, changing rollIncreasing/Decreasing - this is important (see above comment)
			isWaving = false;
